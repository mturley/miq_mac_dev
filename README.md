...
2. Install `rvm` (https://rvm.io/) and then run `rvm install 2.4.2` (Note: the helper scripts all require `rvm`. If you want to use `rbenv` or something else, you'll just need to change the top of each script. The important thing is that you load ruby 2.4.2)
